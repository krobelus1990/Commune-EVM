...
const comtracts = require("./src/routes/comtract.route");
...
// sets default route
app.get("/", (req, res) => {
  res.status(200).json({ message: "Welcome to Comtract Node.js application backend." });
});

// api routes
app.use(process.env.APP_API_PREFIX, comtracts);
...
app.listen(process.env.APP_PORT, () => {
  console.log("Comtract backend server running on: " + process.env.APP_BASE_URL);
});
